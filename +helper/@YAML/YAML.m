classdef YAML
    ...
    methods (Static)
        ...
        function [ X ] = load( S )
            ...
            javaaddpath(helper.YAML.JARFILE);
            ...
            X = helper.YAML.load_data(java_obj);
        end
        ...
        function [ S ] = dump( X )
            ...
            yaml = org.yaml.snakeyaml.Yaml();
            java_obj = helper.YAML.dump_data(X);
    ...
    methods(Static, Access=private)
        function result = load_data( r )
            ...
            elseif isa(r, 'double')
                result = double(r);
            % Moidfied: added parsing of logical values
            ...
            elseif isa(r, 'java.util.List')
                ...
                i = 1;
                while itr.hasNext()
                    result{i} = helper.YAML.load_data(itr.next());
                    i = i + 1;
                end
                result = helper.YAML.merge_cell(result);
            elseif isa(r, 'java.util.Map')
                result = struct;
                itr = r.keySet().iterator();
                while itr.hasNext()
                    key = itr.next();
                    result.(char(key)) = helper.YAML.load_data(...
        ...
        function result = dump_data( r )
            ...
            elseif ~isscalar(r)
                ...
                if size(r,1)==1
                    for i = 1:numel(r)
                        if iscell(r)
                            result.add(helper.YAML.dump_data(r{i}));
                        else
                            result.add(helper.YAML.dump_data(r(i)));
                        end
                    end
            ...
            elseif isstruct(r)
                ...
                keys = fields(r);
                for i = 1:length(keys)
                    result.put(keys{i},helper.YAML.dump_data(r.(keys{i})));
                end
            elseif iscell(r)
                result = java.util.ArrayList();
                result.add(helper.YAML.dump_data(r{1}));
